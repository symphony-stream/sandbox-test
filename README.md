...
The container sits on an Incus network that has no route out; the only
thing it can reach there is an allowlist proxy (plain
[mitmproxy](https://mitmproxy.org/), no custom code) running in the VM. Your `~/sandbox` and `~/.claude` are mounted
...
## What it protects
The container sees only the mounted folders. Its network is an Incus
bridge with routing and NAT off, so the only address it can reach is the
VM's, where the proxy listens; the proxy reaches only the hosts you list
and never your machine (an unlisted host gets a 403 without any
connection opening). Root inside the container cannot change that: the
network, the proxy and the VM are outside it. `config.yaml` with its tokens is
